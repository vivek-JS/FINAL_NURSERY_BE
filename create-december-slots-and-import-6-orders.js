...
const createMissingSlots = async () => {
  try {
    ...
    console.log('\n🔧 Creating missing slots for December 2025...');
    ...
    const rows801_900 = allData.slice(800, 900);
    const bookingNumbers = ['25-26/B0198', '25-26/B0221', '25-26/B0235', '25-26/B0237', '25-26/B0238', '25-26/B0261'];
    
    const neededSlots = new Map(); // Map: "plantName::varietyName" -> { plantName, varietyName }

    rows801_900.forEach((row) => {
    ...
    for (const [key, { plantName, varietyName }] of neededSlots.entries()) {
      console.log(`\n🔧 Creating slots for ${plantName} -> ${varietyName} (December 2025)...`);
      ...
      // Filter to only December slots
      const decemberSlots = slots2025.filter(slot => {
        const slotDate = moment(slot.startDay, 'DD-MM-YYYY');
        return slotDate.month() === 11; // December is month 11 (0-indexed)
      });

      const slotTemplates = decemberSlots.map(slot => ({
        ...slot,
      ...
      console.log(`   ✅ Created/updated slots for ${plantName} -> ${varietyName} (December 2025)`);
    }
...
const import6Orders = async () => {
  try {
    ...
    // Find the 6 specific rows by booking number
    const bookingNumbers = ['25-26/B0198', '25-26/B0221', '25-26/B0235', '25-26/B0237', '25-26/B0238', '25-26/B0261'];
    const rows801_900 = allData.slice(800, 900);
    
    const targetRows = [];
    rows801_900.forEach((row, idx) => {
      const bookingNo = (row['Booking NO.'] || row['Booking\r\nNO.'] || '').toString();
      if (bookingNumbers.includes(bookingNo)) {
        targetRows.push(row);
        console.log(`   Found: Row ${801 + idx}, Booking: ${bookingNo}`);
      }
    });
    
    if (targetRows.length === 0) {
      console.log('❌ Could not find the 6 orders in rows 801-900');
      await mongoose.disconnect();
      return;
    }
    
    // Create a new workbook with only these 6 rows
