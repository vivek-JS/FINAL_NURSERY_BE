...
const importAllOrders = async () => {
  try {
    ...
    const rowLimit = null; // Import all rows
    
    console.log(`\n📥 Importing ALL orders from Excel file...\n`);
    ...
    if (results.errorfulOrders && results.errorfulOrders.length > 0) {
      console.log(`\n📋 Errorful Orders Stored: ${results.errorfulOrders.length}`);
      console.log(`   💡 You can retry these orders after fixing issues`);
    }
