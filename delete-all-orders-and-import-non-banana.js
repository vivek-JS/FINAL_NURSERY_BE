...
const deleteAllOrdersAndImportNonBanana = async () => {
  try {
    await connectDB();
    ...
    console.log(`   Excluded: ${jsonData.length - nonBananaRows.length} Banana rows`);
    ...
    if (nonBananaRows.length === 0) {
      console.log('\n⚠️  No rows to import (all rows are Banana)');
      await mongoose.connection.close();
      return;
    }
    ...
    const importResults = await importOrdersAndFarmers(excelBuffer, {
      sourceFilename: 'non-banana-orders.xlsx'
    });
    ...
    if (importResults.success.length > 0) {
      ...
      importResults.success.forEach(success => {
        // Try to find crop from the original data
        const originalRow = nonBananaRows.find(r => 
          (r['Booking NO.'] && success.bookingNo && r['Booking NO.'].toString() === success.bookingNo.toString()) ||
          (r.Name && success.farmerName && r.Name === success.farmerName)
        );
        const crop = originalRow?.Crop || 'Unknown';
        successByCrop[crop] = (successByCrop[crop] || 0) + 1;
      });
      
      console.log('\n✅ Successful Imports by Crop:');
      Object.entries(successByCrop).forEach(([crop, count]) => {
        console.log(`   ${crop}: ${count} orders`);
      });
    }
    ...
    if (importResults.errors.length > 0) {
      ...
      importResults.errors.slice(0, 5).forEach((error, idx) => {
        console.log(`   ${idx + 1}. Row ${error.row || 'N/A'}: ${error.error || 'Unknown error'}`);
      });
    }
...
deleteAllOrdersAndImportNonBanana();
