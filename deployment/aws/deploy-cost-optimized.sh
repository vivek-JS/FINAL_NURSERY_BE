...
build_and_deploy_lambda() {
    ...
    # Copy backend application files from FINAL_NURSERY_BE
    cp -r /Users/VivekP/FINAL_NURSERY_BE/* "$TEMP_DIR/"
    cd "$TEMP_DIR"
    ...
    # Rebuild bcrypt for Linux (Lambda environment)
    npm rebuild bcrypt --target_platform=linux --target_arch=x64
