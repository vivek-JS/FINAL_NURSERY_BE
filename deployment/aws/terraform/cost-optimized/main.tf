...
terraform {
  ...
  backend "s3" {
    bucket = "nursery-terraform-state-039612884746"
    key    = "nursery-cost-optimized/terraform.tfstate"
    region = "us-east-1"
  }
}

# AWS Provider for ap-south-1 (main region)
provider "aws" {
...
# AWS Provider for us-east-1 (required for CloudFront)
provider "aws" {
  alias  = "us-east-1"
  region = "us-east-1"
...
resource "aws_api_gateway_method" "root" {
  ...
  resource_id   = aws_api_gateway_rest_api.nursery_api.root_resource_id
  http_method   = "ANY"
  authorization = "NONE"
}
...
resource "aws_api_gateway_stage" "nursery_api" {
  deployment_id = aws_api_gateway_deployment.nursery_api.id
  rest_api_id   = aws_api_gateway_rest_api.nursery_api.id
...
resource "aws_api_gateway_deployment" "nursery_api" {
  ...
  # Force new deployment when changes are made
  lifecycle {
...
resource "aws_cloudfront_distribution" "frontend" {
  count = 0  # Temporarily disabled until certificate is validated
...
resource "aws_cloudfront_origin_access_identity" "frontend" {
  count   = var.domain_name != "your-nursery-domain.com" ? 1 : 0
  comment = "OAI for ${var.project_name} frontend"
}

# DocumentDB cluster (cost-optimized with smallest instance)
...
resource "aws_docdb_cluster_instance" "cluster_instances" {
  count              = 1  # Cost optimization: single instance
  identifier         = "${var.project_name}-docdb-${count.index}"
  cluster_identifier = aws_docdb_cluster.main.id
  instance_class     = "db.r5.large"  # Supported instance class for DocumentDB
...
# ElastiCache Redis (cost-optimized with smallest instance)
resource "aws_elasticache_replication_group" "redis" {
  replication_group_id          = "${var.project_name}-redis"
  description                   = "Redis cluster for ${var.project_name}"
  node_type                     = "cache.t3.micro"  # Smallest instance for cost optimization
  port                          = 6379
  parameter_group_name          = "default.redis7"
  subnet_group_name             = aws_elasticache_subnet_group.redis.name
  security_group_ids            = [aws_security_group.redis.id]
  num_cache_clusters            = 1  # Cost optimization: single node
...
resource "aws_route53_record" "frontend" {
  count = 0  # Temporarily disabled until CloudFront is ready
...
resource "aws_route53_record" "api" {
  count = 0  # Temporarily disabled until API Gateway is ready
  zone_id = var.hosted_zone_id
  name    = "api.${var.domain_name}"
  type    = "A"

  alias {
    name                   = aws_api_gateway_stage.nursery_api.invoke_url
    zone_id                = aws_api_gateway_rest_api.nursery_api.arn
    evaluate_target_health = false
  }
}
...
# ACM Certificate for regional services
...
resource "aws_cloudwatch_log_group" "lambda" {
  name              = "/aws/lambda/${aws_lambda_function.nursery_api.function_name}"
...
resource "aws_budgets_budget" "cost" {
  count = 0  # Temporarily disabled since it already exists
