...
export const validateExcelStructure = (buffer) => {
  ...
  if (missingColumns.length > 0) {
    validationResults.isValid = false;
    validationResults.errors.push(`Missing required columns: ${missingColumns.join(', ')}`);
  }
  ...
  data.forEach((row, index) => {
    ...
    if (!row['Booking NO.'] || !/^\d{2}-\d{2}\/B\d+$/.test(row['Booking NO.'])) {
      rowErrors.push('Invalid booking number format (should be like "24-25/B1392")');
    }

    // Validate mobile number
    if (!row.mobileNumber || !/^\d{10}$/.test(row.mobileNumber.toString())) {
      rowErrors.push('Invalid mobile number (should be 10 digits)');
    }

    // Validate quantities
    if (!row.numberPlants || isNaN(row.numberPlants) || Number(row.numberPlants) <= 0) {
  ...
  for (const row of data) {
    ...
    try {
      ...
      const processedRow = {
        ...
        'Advance Date': row['Advance Date'] ? convertDate(row['Advance Date']) : null
      };

      // Check if order already exists
      const orderNumber = processedRow['Booking NO.'].replace('24-25/B', '');
      const existingOrder = await Order.findOne({ orderId: orderNumber }).session(session);
      
      if (existingOrder) {
        throw new Error(`Order with booking number ${processedRow['Booking NO.']} already exists`);
      }

      // Create/update farmer
      const farmerData = {
        name: processedRow.name,
        mobileNumber: processedRow.mobileNumber,
      ...
      let farmer = await Farmer.findOne({ mobileNumber: farmerData.mobileNumber }).session(session);
      if (!farmer) {
      ...
      // Get sales person
      const salesPerson = await User.findOne({ name: processedRow.orderBy }).session(session);
      if (!salesPerson) {
        throw new Error(`Sales person "${processedRow.orderBy}" not found`);
      }

      // Find plant and variety
      const plant = await PlantCms.findOne({ name: processedRow.plantName }).session(session);
      if (!plant) {
        throw new Error(`Plant type "${processedRow.plantName}" not found`);
      }

      const subtype = plant.subtypes.find(st => st.name === processedRow.plantSubtype);
      if (!subtype) {
        throw new Error(`Variety "${processedRow.plantSubtype}" not found for ${processedRow.plantName}`);
      }
      ...
      // Calculate amounts
      const totalAmount = Number(processedRow.numberPlants) * Number(processedRow.Rate);
...
async function findDeliverySlot(plantId, subtypeId, deliveryDate, session) {
  try {
    ...
    if (!targetSlot) {
      throw new Error(`No suitable slot found for delivery date ${deliveryMoment.format('DD-MM-YYYY')} in month ${month}`);
    }
